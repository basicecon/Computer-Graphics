...
void TMesh::RenderFilled(PPC *ppc, FrameBuffer *fb, FrameBuffer *texture, 
	unsigned int color, V3 L, float ka, float es, int renderMode, int lookupMode, int tilingMode) {

	

	//"C:\Users\Kathy Wang\Documents\Visual Studio 2013\Projects\InteractiveGraphics\texture.tiff";
	/*
	...
	if (!input.empty()) {
		...
		texture[1] = new FrameBuffer(0, 0, 0, 0);
		TIFF* tiff = fb->LoadTiff(fileName, texture[1]);
		delete[] fileName;
		texture[1]->show();
	}*/
